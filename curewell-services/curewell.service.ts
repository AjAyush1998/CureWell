...
export class CurewellService {
  ...
  getAllSpecializations(): Observable<Specialization[]> {
   //To do implement necessary logic
    let tem = this.http.get<Specialization[]>('http://localhost:50476/api/CureWell/GetSpecializations').pipe(catchError(this.errorHandler));
    return tem;
  }
  ...
  getAllSurgeriesForToday(): Observable<Surgery[]> {
    //To do implement necessary logic
    let temS = this.http.get<Surgery[]>('http://localhost:50476/api/CureWell/GetAllSurgeryTypeForToday').pipe(catchError(this.errorHandler));
    return temS;
  }
  ...
  editDoctorDetails(doctorId: number, doctorName: string): Observable<boolean> {
    ...
    return this.http.put<boolean>('http://localhost:50476/api/CureWell/UpdateDoctorDetails', docObj).pipe(catchError(this.errorHandler));
  }
